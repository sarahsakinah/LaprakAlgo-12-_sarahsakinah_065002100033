...
def binary_search(nomor,mark_dicari,kiri,kanan):
    while kiri <= kanan:
        mid = (kiri + kanan)//2
...
results = binary_search(nomor,mark_dicari,0,len(nomor)-1)
if results != -1:
    print("hasil binary search ditemukan pada index ke " + str(results))
else:
